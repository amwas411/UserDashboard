...
interface SysAdminUnit extends Entity {
	name?: string,
	userPassword?: string,
	active?: boolean,
...
let updatingUserId: string = "";
let userDialogMode: number = USER_DIALOG_MODES.UNSET;
...
function getUserPostPayload(formId: string): SysAdminUnit {
	const form = document.getElementById(formId) as HTMLFormElement;
	if (!form) {
		throw new Error(`#${formId} not found`);
	}
	
	let payload: SysAdminUnit = {};
	...
	payload.active = BOOL_VALUES_MAP.get(formData.get("active") as string);
	return payload;
}
...
async function deleteUser(elementId: string): Promise<void> {
...
function createTable(data: Entity[], columns: Column[], tableId: string): HTMLTableElement {
...
async function createUsersTable(response: void | Response): Promise<void> {
	...
	let data = await response.json() as SysAdminUnit[];
	let table = createTable(data, USER_TABLE_COLUMNS.filter((i) => !EXCLUDED_TABLE_COLUMN_NAMES.includes(i.name)), 
		USER_TABLE_ID);
...
function appendRowButtons(tr: HTMLTableRowElement, element: SysAdminUnit): void {
...
function getDeleteButton(elementId: string): HTMLButtonElement {
...
function getUpdateButton(element: SysAdminUnit): HTMLButtonElement {
...
function getEntityColumnValue(element: Entity, columnName: string): string {
	let elementValue = "";
...
function setDialogVisible(dialogId: string, isVisible: boolean) {
...
function showDialog(dialogId: string, defaultValues?: Map<string, boolean>): void {
	prepareDialogColumns(dialogId, defaultValues);
	setDialogVisible(dialogId, true);
}

function prepareDialogColumns(dialogId: string, defaultValues?: Map<string, boolean>): void {
	let inputFields = document.querySelectorAll<HTMLInputElement>(`#${dialogId} form .field input`);
